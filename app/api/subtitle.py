...
from ..services.subtitle import find_cue_index, parse_srt
...
def bilibili_subtitle(req: BilibiliSubtitleRequest):
    ...
    url = meta.get("url")
    if not url:
        raise HTTPException(400, "该视频不是 B站来源，无法拉取官方字幕")
    subs = bilibili.fetch_subtitles(url)
    if req.lang and req.lang in subs:
        cues = subs[req.lang]
        lang = req.lang
    else:
        # 默认优先中文字幕（ai-zh / zh-CN 等）
...
@router.get("/{video_id}/at")
def subtitle_at(video_id: str, t: float):
    """当前时间 → 字幕索引。"""
    cues = state.load_subtitles(video_id)
    return {"index": find_cue_index(cues, t)}
