...
- **视频**：打开本地视频（浏览器内播放，不必先上传）；B站链接解析（yt-dlp），支持多P选集与清晰度选择，在线串流播放（未登录 360p~720p）。
- **字幕**：SRT 上传、B站官方字幕（需 cookies）、ASR 实时转录（边看边转）三种来源；列表与播放进度双向同步，点击跳转。
- **AI 问答**：OpenAI 兼容协议（DeepSeek/OpenAI），多轮对话按视频保存在浏览器、视频总结注入、可选「发送当前字幕」「发送当前视频帧」「思考」，SSE 流式输出。「⚙ 对话设置」可配置是否注入视频总结/完整字幕、当前字幕前后携带条数；每次回复会展示本次实际注入的上下文，可展开核对。
...
- **用户库（浏览器）**：视频列表、字幕、总结、对话、打开过的本地视频文件。IndexedDB + OPFS，换浏览器或清站点数据会丢。
- **工作缓存（`data/`，已 gitignore）**：LLM/ASR 配置、B站 cookies、串流代理用的地址、转录用的音频/暂存媒体、HLS 分片。后端不当作用户库；清掉后列表还在，B站视频重新解析即可播放。
